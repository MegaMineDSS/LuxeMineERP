...
JobSheetWidget::JobSheetWidget(QWidget *parent)
    : QWidget(parent), ui(new Ui::JobSheetWidget) {
  ui->setupUi(this);
  ...
  if (f.pointSizeF() > 0) {
    ...
    f.setPixelSize(static_cast<int>(f.pixelSize() * scaleFactor));
  }
  setFont(f);

  // Apply scaled margins
  ui->gridLayout->setContentsMargins(
      static_cast<int>(4 * scaleFactor), static_cast<int>(4 * scaleFactor),
      static_cast<int>(4 * scaleFactor), static_cast<int>(4 * scaleFactor));
  ...
  auto scaleTable = [this](QTableWidget *table) {
    table->horizontalHeader()->setDefaultSectionSize(static_cast<int>(
        table->horizontalHeader()->defaultSectionSize() * scaleFactor));
    table->verticalHeader()->setDefaultSectionSize(static_cast<int>(
        table->verticalHeader()->defaultSectionSize() * scaleFactor));
  ...
  if (userRole == "designer") {
    set_value_designer();
    connect(ui->desigNoLineEdit, &QLineEdit::returnPressed, this,
            &JobSheetWidget::loadImageForDesignNo);
  } else if (userRole == "manufacturer") {
    set_value_manuf();
  }
  // qDebug() << ui->jobNoLineEdit->text();
}

void JobSheetWidget::setUserRole(const QString &role) {
  userRole = role;
  if (userRole == "designer") {
    set_value_designer();
    connect(ui->desigNoLineEdit, &QLineEdit::returnPressed, this,
...
JobSheetWidget::~JobSheetWidget() { delete ui; }
...
void JobSheetWidget::resizeEvent(QResizeEvent *event) {
  ...
  if (!originalPixmap.isNull()) {
    ui->productImageLabel->setPixmap(
        originalPixmap.scaled(ui->productImageLabel->size() * scaleFactor,
...
void JobSheetWidget::set_value(const QString &jobNo) {
  bool ok = false;
  int jobId = jobNo.toInt(&ok);
  if (!ok) {
    qDebug() << "Invalid jobNo (not an int):" << jobNo;
    // Optionally handle error
  }

  auto dataOpt = DatabaseUtils::fetchJobSheetData(jobId);
  if (!dataOpt) {
    qDebug() << "No record found for jobId:" << jobId;
    return;
  }
  ...
  ui->dateOrderDateEdit->setDate(
      QDate::fromString(data.orderDate, "yyyy-MM-dd"));
  ui->deliDateDateEdit->setDate(
      QDate::fromString(data.deliveryDate, "yyyy-MM-dd"));
  ...
  auto [diamondJson, stoneJson] =
      DatabaseUtils::fetchDiamondAndStoneJson(data.designNo);

  QTableWidget *table = ui->diaAndStoneForDesignTableWidget;
  table->setRowCount(0);
  table->setColumnCount(6);
  table->setHorizontalHeaderLabels(
      {"Type", "Name", "Quantity", "Size (MM)", "1 Pc Wt", "Total Wt"});
  ...
  auto parseAndAddRows = [&](const QString &jsonStr, const QString &typeLabel) {
    ...
    for (auto value : doc.array()) {
      ...
      table->setItem(row, 4,
                     new QTableWidgetItem(QString::number(wtEach, 'f', 3)));
      table->setItem(row, 5,
                     new QTableWidgetItem(QString::number(wtTotal, 'f', 3)));
  ...
  // ✅ Refresh Manufacturer Tables (Issue/Return/Broken)
  // This ensures that even if updateDiamondTotals ran in constructor (with
  // empty jobNo), it runs again now that we have the valid Job No set.
  updateDiamondTotals();
...
void JobSheetWidget::loadImageForDesignNo() {
  ...
  // ✅ ALWAYS save the design no into OrderBook-Detail, even if image not found
  saveDesignNoAndImagePath(designNo, imagePath);

  if (imagePath.isEmpty()) {
    QMessageBox::information(this, "Not Found",
                             "No image found for design number: " + designNo);
    // Even if no image, we saved the designNo.
    // Clear the current image label? Maybe
    ui->productImageLabel->clear();
    return;
  }

  QString fullPath =
      QDir::cleanPath(QCoreApplication::applicationDirPath() + "/" + imagePath);
  QPixmap pixmap(fullPath);

  if (!pixmap.isNull()) {
    ui->productImageLabel->setPixmap(
        pixmap.scaled(ui->productImageLabel->size(), Qt::KeepAspectRatio,
...
void JobSheetWidget::saveDesignNoAndImagePath(const QString &designNo,
                                              const QString &imagePath) {
  QString jobNo = ui->jobNoLineEdit->text().trimmed();
  if (jobNo.isEmpty()) {
    QMessageBox::warning(this, "Missing Data",
                         "Job No is missing. Cannot save image path.");
    return;
  }
  ...
  int jobId = jobNo.toInt(&ok);
  if (!ok) {
    QMessageBox::warning(this, "Data Error", "Job No is not a valid integer.");
    return;
  }

  if (!DatabaseUtils::updateDesignNoAndImagePath(jobId, designNo, imagePath)) {
    QMessageBox::critical(this, "Query Error",
                          "Failed to update order_book_detail.");
  }
}
...
void JobSheetWidget::set_value_designer() {
  ...
  for (QLineEdit *edit : lineEdits) {
    if (edit == ui->desigNoLineEdit && userRole == "designer") {
  ...
  for (QTableWidget *table : tableWidgets) {
    ...
    table->setStyleSheet(
        "QTableWidget { background-color: #f0f0f0; }"); // optional visual cue
  }
...
void JobSheetWidget::set_value_manuf() {
  ...
  // ---------------------- GOLD TABLE ----------------------
  //
  ui->goldDetailTableWidget->setSelectionMode(
      QAbstractItemView::SingleSelection);
  ui->goldDetailTableWidget->setSelectionBehavior(
      QAbstractItemView::SelectItems);

  updateGoldTotalWeight();

  // ✅ Open ManageGold for col 1 (Filling), col 2 (Dust), col 4 (Return)
  connect(ui->goldDetailTableWidget, &QTableWidget::cellClicked, this,
          [this](int row, int col) {
            if (row == 0 && (col == 1 || col == 2 || col == 4)) {
              QTableWidgetItem *item =
                  ui->goldDetailTableWidget->item(row, col);
              if (!item) {
  ...
  ui->goldDetailTableWidget->setEditTriggers(QAbstractItemView::DoubleClicked |
                                             QAbstractItemView::EditKeyPressed);

  connect(ui->goldDetailTableWidget, &QTableWidget::cellChanged, this,
          [this](int row, int col) {
  ...
  // ---------------------- DIAMOND & STONE TABLE ----------------------
  //
  ui->diamondAndStoneDetailTableWidget->setSelectionMode(
      QAbstractItemView::SingleSelection);
  ui->diamondAndStoneDetailTableWidget->setSelectionBehavior(
      QAbstractItemView::SelectItems);

  // ✅ Make every cell non-editable
  for (int r = 0; r < ui->diamondAndStoneDetailTableWidget->rowCount(); ++r) {
    for (int c = 0; c < ui->diamondAndStoneDetailTableWidget->columnCount();
         ++c) {
...
void JobSheetWidget::onGoldDetailCellClicked(QTableWidgetItem *item) {
  ...
  if (row == 0 && (col == 1 || col == 2 || col == 4)) {
    if (!newManageGold) {
      ...
      connect(newManageGold, &ManageGoldDialog::totalWeightCalculated, this,
              [this](double weight) {
                int r = 0;
                int c =
                    (newManageGold->currentMode == ManageGoldDialog::Filling)
                        ? 1
                        : 4;
    ...
    newManageGold->setMode(col == 1 ? ManageGoldDialog::Filling
                           : col == 4
                               ? ManageGoldDialog::Returning
                               : ManageGoldDialog::Dust // ✅ new mode for dust
    );

    QRect cellRect = ui->goldDetailTableWidget->visualItemRect(item);
    QPoint globalPos = ui->goldDetailTableWidget->viewport()->mapToGlobal(
        cellRect.bottomLeft());
...
void JobSheetWidget::updateGoldTotalWeight() {
  ...
  // ✅ Use DatabaseUtils
  auto totals = DatabaseUtils::fetchGoldTotals(jobNo);
  ...
  auto getProductWeight = [](const QString &returnJson) -> double {
    ...
    QJsonParseError parseError;
    QJsonDocument doc =
        QJsonDocument::fromJson(returnJson.toUtf8(), &parseError);
    if (parseError.error != QJsonParseError::NoError || !doc.isArray())
      return 0.0;
  ...
  double totalIssueWeight = totals.totalIssue;
  double totalReturnWeight = totals.totalReturn;
  double dustWeight = totals.dustWeight;

  // Update Dust Row (Row 0, Col 2)
  if (dustWeight > 0.0) {
    int row = 0, col = 2;
  ...
  setCell(1, 4, totals.buffingReturn);
  setCell(2, 4, totals.freePolishReturn);
  setCell(3, 4, totals.settingReturn);
  setCell(4, 4, totals.finalPolishReturn);
  ...
  double productWeight = getProductWeight(totals.returnJson);
  ...
  auto calcLossPercent = [this](int row, double adjustExtra = 0.0) {
    ...
    if (!inputItem || !returnItem)
      return;

    double input = inputItem->text().toDouble();
    double ret =
        returnItem->text().toDouble() - adjustExtra; // subtract extra weights
  ...
  // --- Overall loss (row 0, including dust) ---
  {
    ...
    double lossPercent =
        (totalIssueWeight > 0.0) ? (loss / totalIssueWeight) * 100.0 : 0.0;
  ...
  // --- Total row (row 5) ---
  {
    ...
    double totalLossPercent =
        (totalIssue > 0.0) ? (lossSum / totalIssue) * 100.0 : 0.0;
  ...
  ui->goldDetailTableWidget->setHorizontalHeaderItem(
      5, new QTableWidgetItem("Loss %"));
  ...
  // --- Gross & Net Weight display ---
  {
    ...
    qDebug() << "Gross Wt:" << grossWt << " | Used DSO:" << totalUsedDSO
             << " | Net Wt:" << netWt;
  }
}
...
void JobSheetWidget::updateDiamondTotals() {
  ...
  // ✅ Use DatabaseUtils to fetch totals

  auto totalsMap = DatabaseUtils::fetchDiamondTotals(jobNo);
  ...
  for (const auto &e : entries) {
    if (!totalsMap.contains(e.col))
      continue;

    auto [pcs, wt] = totalsMap[e.col];

    QTableWidgetItem *pcsItem =
        ui->diamondAndStoneDetailTableWidget->item(e.row, e.pcsCol);
    if (!pcsItem) {
    ...
    QTableWidgetItem *wtItem =
        ui->diamondAndStoneDetailTableWidget->item(e.row, e.wtCol);
    if (!wtItem) {
...
// call this from set_value_manuf() to enable only diamond-issue cells (0,1) and
// (0,2)
void JobSheetWidget::setupDiamondIssueClicks() {
  // Use UniqueConnection to avoid multiple identical connects if
  // set_value_manuf() runs again
  connect(ui->diamondAndStoneDetailTableWidget, &QTableWidget::cellClicked,
          this, [this](int row, int col) {
            ...
            QTableWidgetItem *item =
                ui->diamondAndStoneDetailTableWidget->item(row, col);
            if (!item) {
              item = new QTableWidgetItem();
            ...
            if (jobNo.isEmpty()) {
              QMessageBox::warning(this, "Missing Job No",
                                   "Please enter or select a valid Job Number "
                                   "before adding diamond data.");
              return;
            }
            ...
            if (!newDiamonIssueRetBro) {
              ...
              newDiamonIssueRetBro->setWindowFlags(Qt::FramelessWindowHint |
                                                   Qt::Popup);
              newDiamonIssueRetBro->setAttribute(Qt::WA_DeleteOnClose, false);

              // hide handling
              connect(newDiamonIssueRetBro,
                      &DiamonIssueRetBroDialog::menuHidden, this, [this]() {
                        qApp->removeEventFilter(this);
              ...
              connect(
                  newDiamonIssueRetBro, &DiamonIssueRetBroDialog::valuesUpdated,
                  this, [this](int r, const QVariantMap &vals) {
                    auto setVal = [this](int rr, int cc, const QString &val) {
                      QTableWidgetItem *cell =
                          ui->diamondAndStoneDetailTableWidget->item(rr, cc);
                      if (!cell) {
                        cell = new QTableWidgetItem();
                        ui->diamondAndStoneDetailTableWidget->setItem(rr, cc,
                                                                      cell);
                      }
                      cell->setText(val);
                    };
                    ...
                    if (vals.contains("issue_pcs"))
                      setVal(r, 1, vals["issue_pcs"].toString());
                    if (vals.contains("issue_wt"))
                      setVal(r, 2, vals["issue_wt"].toString());
                    if (vals.contains("return_pcs"))
                      setVal(r, 3, vals["return_pcs"].toString());
                    if (vals.contains("return_wt"))
                      setVal(r, 4, vals["return_wt"].toString());
                    if (vals.contains("broken_pcs"))
                      setVal(r, 5, vals["broken_pcs"].toString());
                    if (vals.contains("broken_wt"))
                      setVal(r, 6, vals["broken_wt"].toString());
            ...
            QRect cellRect =
                ui->diamondAndStoneDetailTableWidget->visualItemRect(item);
            QPoint globalPos =
                ui->diamondAndStoneDetailTableWidget->viewport()->mapToGlobal(
                    cellRect.bottomLeft());
...
void JobSheetWidget::handleCellSave(int row, int col) {
  ...
  QString dbColumn;
  if (row == 1 && col == 4)
    dbColumn = "buffing_return";
  else if (row == 2 && col == 4)
    dbColumn = "free_polish_return";
  else if (row == 3 && col == 4)
    dbColumn = "setting_return";
  else if (row == 4 && col == 4)
    dbColumn = "final_polish_return";
  else
  ...
  // ✅ Check if already exists logic could be moved to DB utils or we trust
  // DatabaseUtils to handle update/insert But wait, the original logic had a
  // specific "Locked" check "if already exists -> Locked" I should preserve
  // that check if it's important policy. However,
  // `DatabaseUtils::fetchGoldTotals` returns the current values.

  auto currentTotals = DatabaseUtils::fetchGoldTotals(jobNo);
  double existingValue = 0.0;
  if (dbColumn == "buffing_return")
    existingValue = currentTotals.buffingReturn;
  else if (dbColumn == "free_polish_return")
    existingValue = currentTotals.freePolishReturn;
  else if (dbColumn == "setting_return")
    existingValue = currentTotals.settingReturn;
  else if (dbColumn == "final_polish_return")
    existingValue = currentTotals.finalPolishReturn;

  if (existingValue > 0.001) { // fuzzy check > 0
    // If the user tries to edit it again?
    // The original code checked if "existingText" was not empty.
    // If we want to simulate the lock:
    if (std::abs(value - existingValue) > 0.001) {
      // Trying to change it?
      // Actually original logic: if exists, revert to existing and warn.
      QMessageBox::warning(
          this, "Locked", "This cell is already filled and cannot be changed.");
  ...
  // ✅ Confirm save
  auto reply =
      QMessageBox::question(this, "Confirm", "Save value " + formatted + " ?");
  if (reply == QMessageBox::Yes) {
    if (DatabaseUtils::saveGoldStageReturn(jobNo, dbColumn, value)) {
