...
#include "designer/modifycatalog.h"

#include <QMdiSubWindow>

DesignerWindow::DesignerWindow(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::DesignerWindow) {
  ui->setupUi(this);

  setWindowTitle("Designer Dashboard");

  connect(ui->actionSwitch_Role, &QAction::triggered, this,
          &DesignerWindow::changeRole);
  connect(ui->actionAdd_Catalog, &QAction::triggered, this,
          &DesignerWindow::openAddCatalog);

  connect(ui->actionOrder_List, &QAction::triggered, this,
          &DesignerWindow::openOrderList);

  connect(ui->actionModify_Catalog, &QAction::triggered, this,
          &DesignerWindow::openModifyCatalog);
...
DesignerWindow::~DesignerWindow() { delete ui; }

QMdiArea *DesignerWindow::mdiArea() const { return ui->mdiArea; }
...
void DesignerWindow::openOrderList() {
  for (QMdiSubWindow *sub : ui->mdiArea->subWindowList()) {
    if (sub->widget() && sub->widget()->objectName() == "DesignerOrderList") {
      ui->mdiArea->setActiveSubWindow(sub);
      return;
    }
  }
  ...
  widget->setObjectName("DesignerOrderList");
